...
interface Job {
  ...
  logs: { id: string; timestamp: string; level: 'info' | 'warn' | 'error'; message: string; step: string }[];
  metadata: {
    fileName: string;
    fileSize: string;
    duration: string;
...
const sseClients = new Map<string, any[]>(); // jobId -> Response objects
...
async function startServer() {
  ...
  async function runBackgroundPipeline(jobId: string, customContent?: string, resumeFromStepIndex = 1) {
    const job = activeJobs.get(jobId);
    if (!job) return;
    ...
    try {
      ...
      for (let i = resumeFromStepIndex - 1; i < stepsList.length; i++) {
        ...
        // Simulate real duration & progress updates
        const duration = 2500; // 2.5s per step
        const ticks = 4;
        for (let t = 1; t <= ticks; t++) {
          await new Promise(resolve => setTimeout(resolve, duration / ticks));
          
          // Check if job is still active or deleted/failed
        ...
        // Simulate failure trigger for retry testing if specified by title or environment
      ...
      // Map real video based on target language
      ...
      // Generate realistic high-fidelity transcript
      let generatedTranscript = [
        { id: 't1', start: 0.0, end: 5.5, speaker: 'Speaker A', text: 'Hello everyone, welcome to the demonstration. Today we show you our fully automated video translator.', translatedText: `[Translated to ${job.targetLanguage}] Bonjour à tous, bienvenue dans la démonstration. Aujourd'hui nous vous présentons notre traducteur vidéo entièrement automatisé.` },
        { id: 't2', start: 5.5, end: 11.2, speaker: 'Speaker A', text: 'This solution lets you extract speech and clone vocal profiles natively in real-time.', translatedText: `[Translated to ${job.targetLanguage}] Cette solution vous permet d'extraire la parole et de cloner des profils vocaux nativement en temps réel.` },
        { id: 't3', start: 11.2, end: 18.0, speaker: 'Speaker B', text: 'Wow, that is incredibly powerful. The synthesis sounds exactly like the original speaker!', translatedText: `[Translated to ${job.targetLanguage}] Wow, c'est incroyablement puissant. La synthèse ressemble exactement à l'orateur d'origine !` }
      ];
  ...
  app.post('/api/start-pipeline', (req, res) => {
    ...
    const job: Job = {
      ...
      logs: [
        { id: 'log-0', timestamp: new Date().toISOString(), level: 'info', message: 'Video file verified. Format and integrity validated.', step: 'Upload Video' }
      ],
      metadata: metadata || {
  ...
  // RETRY PIPELINE (Resumes from the exact failed step!)
  app.post('/api/retry-pipeline', (req, res) => {
    ...
    runBackgroundPipeline(job.id, undefined, resumeIndex);
    res.json({ success: true, resumingFrom: failedStepName });
  });

  // SSE PIPELINE STATUS CONNECT
  ...
  // AUTO LANGUAGE DETECTION EXCEL FILE
  app.post('/api/detect-language', (req, res) => {
    const { filename, sampleText } = req.body;
    
    // Determine language based on common keywords or random distribution
    ...
    if (fn.includes('french') || fn.includes('paris') || txt.includes('bonjour') || txt.includes('oui')) {
      ...
      // Simulate lower confidence on completely random files to trigger low confidence modal
  ...
  // VIDEO COMPREHENSION / ANALYZER (gemini-3.1-pro-preview)
...
*Note: Please configure a valid \`GEMINI_API_KEY\` in Secrets to see the real-time Gemini Pro analysis output.*`
  ...
  // AUDIO TRANSCRIPTION WITH MICROPHONE (gemini-3.5-flash)
  ...
  // GEMINI CHATBOT (gemini-3.1-pro-preview / gemini-3.5-flash / gemini-3.1-flash-lite)
  ...
  // MOUNT WEBSOCKET SERVER FOR GEMINI LIVE API
